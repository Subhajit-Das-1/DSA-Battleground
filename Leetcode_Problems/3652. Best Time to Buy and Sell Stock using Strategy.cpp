...
public:
    long long maxProfit(vector<int>& prices, vector<int>& strategy, int k) {
        ...
        vector<long long> prefProfit(n + 1, 0), prefPrice(n + 1, 0);
        ...
        long long originalProfit = prefProfit[n];
        long long maxGain = 0;
        ...
        for (int l = 0; l + k <= n; l++) {
            ...
            long long oldWindowProfit =
                prefProfit[r] - prefProfit[l];

            // New contribution after modification:
            // first k/2 → 0
            // last k/2 → prices[i]
            long long newWindowProfit =
                prefPrice[r] - prefPrice[l + half];

            long long gain = newWindowProfit - oldWindowProfit;
